# -*- coding: utf-8 -*-
"""
Created on Mon Jul  3 19:51:35 2023
...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 29 21:00:23 2023
...
class SupConTresNet(nn.Module):
    def __init__(self, name='tresnet', head='mlp', feat_dim = 1024,num_classes = num_classes, use_pretrained = True):
        ...
        self.classifier =  list(model_tresnet.children())[-1]
        ...
        for phase in ['train', 'val']:
            ...
            for images, labels_ in dataloaders[phase]:
                ...
                with autocast(enabled=fp16_precision):
                    if phase == 'train':
                        features, predictions = model(images,phase)
                        #print(predictions.size())
                        logits, labels = info_nce_loss(features)
                        loss = criterion(logits, labels, predictions, labels_,phase)
                    else:
                        predictions = model(images,phase)
                        #print(predictions.size())
                        loss = criterion(None, None, predictions, labels_,phase)
...
def initialize_model(num_classes, use_pretrained=True):
    ...
    input_size = 0
    input_size = 384
    model_ft = SupConTresNet(num_classes = num_classes)
...
# Initialize the model for this run
    ...
    torch.save(model_ft.state_dict(), 'tresnet_contrastive_HOF.pth')
