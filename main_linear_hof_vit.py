...
from networks.network_vit import LinearViT
...
def set_model(opt):
    ...
    classifier = LinearViT(name=opt.model, num_classes=opt.n_cls)
    classifier.load_state_dict(torch.load('/content/gdrive/My Drive/epoch_14_classifier_HOF_supcon_crop.pth', map_location='cpu'))
...
def train(train_loader, classifier, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        torch.save(classifier.state_dict(), '/content/gdrive/My Drive/epoch_'+str(epoch)+'_classifier_HOF_supcon_crop.pth')
...
def main():
    ...
    # training routine
    for epoch in range(15, opt.epochs + 1):
